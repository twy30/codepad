...
git config --global alias.id '!myGitAlias_id() { cd "$GIT_PREFIX"; if [ ! -e "$1" ]; then return 0; fi; git ls-tree --full-name --long HEAD "$1"; echo "`date --date=@"$(git log --format=tformat:%ct --max-count=1 -- "$1")" --iso-8601=seconds --utc` `git log --format=tformat:%H --max-count=1 -- "$1"`"; }; myGitAlias_id'
