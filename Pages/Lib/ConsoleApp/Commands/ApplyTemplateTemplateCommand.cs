...
{
    public class ApplyTemplateTemplateCommand : AbstractCommand
    {
        public string TemplatePath { get; }
        public string[] PagePaths { get; }

        public ApplyTemplateTemplateCommand(string[] arguments) : base(arguments)
        {
        ...
        {
            var template = new TemplatePage(File.ReadAllText(TemplatePath));
            foreach (var pagePath in PagePaths)
            {
                var pageBuilder = new StringBuilder();
                pageBuilder.Append(template.PageStart);
                var page = new TemplatePage(File.ReadAllText(pagePath));
                foreach (var marker in TemplatePage.Markers)
                {
                    pageBuilder
